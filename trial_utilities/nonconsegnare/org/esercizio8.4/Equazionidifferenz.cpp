...
vector<double> Pendolo::Eval(double t, const vector<double> &a) const
{
    vector<double> dxdt{a[1], -m_g / m_l * sin(a[0])};
    return dxdt;
}

vector<double> Forzato::Eval(double t, const vector<double> &a) const
{
    vector<double> dxdt{a[1], -m_omega0*m_omega0*a[0]-m_A*a[1]+sin(m_f*t)};
    return dxdt;
}
