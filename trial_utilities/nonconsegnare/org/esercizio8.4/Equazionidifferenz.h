...
class Forzato: public Funzionevettbase {
    public:
    Forzato(double m, double A, double f) {m_omega0=m; m_A=A; m_f=f;};
    Forzato() {;};

    vector<double> Eval(double t, const vector<double> &x) const override;
    double Eval(double t, const double x) const;

    private:
    double m_omega0; // frequenza naturale
    double m_A; // costante di smorzamento
    double m_f; // frequenza di forzamento
};
